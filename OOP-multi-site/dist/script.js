...
window.addEventListener('DOMContentLoaded', function () {
  var slider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_5__["default"]({
    container: '.page',
    btns: '.next'
  });
  slider.render();
  var modulePageSlider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_5__["default"]({
    container: ".moduleapp",
  ...
  var showUpSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_6__["default"]({
    //мини слайдер на 1 слайде index.html
  ...
  showUpSlider.init();
  var modulesSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_6__["default"]({
    //слайдер с 3 слайда
  ...
  modulesSlider.init();
  var feedSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_6__["default"]({
    //feedback - слайдер с отзывами клиентов на 5 слайде 
  ...
  feedSlider.init();
  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_3__["default"]('.showup .play', '.overlay').init(); //модалка с видео

  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_3__["default"]('.module__video-item .play', '.overlay').init();
  new _modules_difference__WEBPACK_IMPORTED_MODULE_0__["default"]('.officerold', '.officernew', '.officer__card-item').init(); //второй слайд

  new _modules_forms__WEBPACK_IMPORTED_MODULE_2__["default"]('.form').init(); //работа с формами и axios запрос на сервер здесь же

  new _modules_showInfo__WEBPACK_IMPORTED_MODULE_4__["default"]('.plus__content').init(); //показ текста при клике (modules.html)

  new _modules_download__WEBPACK_IMPORTED_MODULE_1__["default"]('.download').init(); //для скачивания при клике на div элемент
});
...
function () {
  function Difference(oldOfficer, newOfficer, items) {
    ...
    //левый правый столб и все их карточки
    try {
  ...
  _createClass(Difference, [{
    ...
    value: function bindTriggers(container, items, counter) {
      container.querySelector('.plus').addEventListener('click', function () {
        if (counter !== items.length - 2) {
          //открывается НЕ последний элемент, -2 тк не считаем 1 из элементов - сам плюсик а второй невидимый
    ...
    value: function hideItems(items) {
      items.forEach(function (item, i, arr) {
        if (i !== arr.length - 1) {
          //кнопка всегда последняя и всегда видема
...
function () {
  function Download(triggers) {
    ...
    this.path = 'assets/img/mainbg.jpg'; //путь ко всем файлам одинаковый по тз
  }
  ...
  _createClass(Form, [{
    ...
    value: function initMask() {
      ...
      function createMask(event) {
        //пишем через function ради своего контекста
        ...
        this.value = matrix.replace(/./g, function (a) {
          //берем каждый символ  и передаем его как аргумент в переменную функции ("a")
        ...
        if (event.type === 'blur') {
          ...
          if (this.value.length == 2) {
            //и при этом инпут остался пустым (+1)
            this.value = ''; //покажем обратно матрицу
          }
        } else {
          //если событие focus или input
      ...
      return regeneratorRuntime.async(function postData$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _context.next = 2;
              return regeneratorRuntime.awrap(axios__WEBPACK_IMPORTED_MODULE_7___default.a.post(url, data));
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        try {
          //только для modules.html
          var blockedElem = btn.closest('.module__video-item').nextElementSibling; //элемент который надо разлочить после просмотра 1го видео

          if (i % 2 == 0) {
            //после норм кнопки всегда идет задезейбленная
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== "true") {
            _this.activeBtn = btn; //для разблокировки видео

            if (document.querySelector('iframe#frame')) {
              //когда хоть 1 раз вызывали плеер до этого
              _this.overlay.style.display = 'flex';

              if (_this.path !== btn.getAttribute('data-url')) {
                //плеер вызывался, но ведео другое
                _this.path = btn.getAttribute('data-url');

                _this.player.loadVideoById({
                  videoId: _this.path
                }); //загружаем новое видео при помощи youtube API
    ...
    value: function bindCloseBtn() {
      ...
      //закрытие модального окна и стоп видео
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        //заменяем div элемент с id frame на плеер
        height: '100%',
        width: '100%',
        videoId: "".concat(url),
        events: {
    ...
    value: function onPlayerStateChange(state) {
      //когда изменяется состояние плеера
      try {
        //только для modules.htnl
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; //элемент который надо разлочить после просмотра видео

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); //svg картинка "▶"

        if (state.data === 0) {
          //если воспроизведение завершенно
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            //если наш след элем заблокирован (из-за класса closed)
  ...
  _createClass(ShowInfo, [{
    ...
    value: function init() {
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          var sibling = btn.closest('.module__info-show').nextElementSibling; //необходимый к показу текст
...
function (_Slider) {
  ...
  //MainSlider - главный слайдер меняющий страницу целиком
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      //отвечает за то куда двигается наш слайдер
      if (n > this.slides.length) {
        //возращаем слайдер в начало при последнем слайде
        this.slideIndex = 1;
      }
    ...
    value: function plusSlides(n) {
      //отвечает за движение взад вперед
      this.showSlides(this.slideIndex += n);
    }
  }, {
    key: "bindTruggers",
    value: function bindTruggers() {
      var _this2 = this;
...
function (_Slider) {
  ...
  //мини слайдеры внутри главного слайдера
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      this.slides.forEach(function (slide) {
        ...
        if (_this2.animate) {
          //если это слайдер с доп. эфектами
      ...
      if (!this.slides[0].closest('button')) {
        //метод closest помимо ближайшего родителя, может вернуть самого себя если селектор совпадет: т.о. если не кнопка
    ...
    value: function nextSlide() {
      //когда переключаем слайд вперед нынешний слайд отправляется в конец списка
      if (this.slides[1].tagName == "BUTTON" && this.slides[2].tagName == "BUTTON") {
        //тк кнопки в 1 массиве со слайдами, то когда они на 2 и 3 позиции мы их переставим (багфикс)
    ...
    value: function init() {
      ...
      try {
        ...
        if (this.autoplay) {
          ...
          [].concat(_toConsumableArray(this.next), _toConsumableArray(this.prev), _toConsumableArray(this.slides)).forEach(function (item) {
            //выкл при наведении мыши на слайдер или кнопки переключения
            item.addEventListener('mouseenter', function () {
              return clearInterval(_this4.intervalId);
            });
...
var Slider = //родитель для всех слайдеров
function Slider() {
  ...
  } catch (e) {} //наши слайды (try - фикс ошибки в modules.html)
